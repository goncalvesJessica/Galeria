...
export default function CameraApp() {
  ...
  async function salvarFoto() {
    const arquivo = await MediaLibrary.createAssetAsync(capturaFoto)
      .then(() => {
        Alert.alert("Salvo com sucesso na galeria!");
      })
      .catch(error => {
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <TouchableOpacity style={{ flexDirection: 'row', top: -25 }}>
          ...
          <EvilIcons name="image" size={50} color="#FFF" onPress={() => setOpen(true)} style={{ marginRight: "35%", left:10 }} />
        ...
        >
          <View style={{ flex: 1, top: 10 }}>
            ...
            <TouchableOpacity style={{ flexDirection: 'row',height:50 }}>
              <EvilIcons name="close"size={50} onPress={() => setOpen(false)} style={{ marginLeft: '20%', marginRight: '40%', color: "red" }} />
              <EvilIcons name="arrow-up" size={50} onPress={salvarFoto} color={"green"} />
            </TouchableOpacity>
